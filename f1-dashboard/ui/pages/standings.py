...
class StandingsPage(ctk.CTkFrame):
    ...
    def load_standings(self):
        ...
        self.status.configure(text="⏳ Loading from API...")
    ...
    def _load_data(self):
        try:
            ...
            # Driver standings - Ergast API
            r1 = requests.get(f"http://ergast.com/api/f1/{year}/driverStandings.json", timeout=10)
            driver_data = r1.json()['MRData']['StandingsTable']['StandingsLists']

            # Constructor standings
            r2 = requests.get(f"http://ergast.com/api/f1/{year}/constructorStandings.json", timeout=10)
            constructor_data = r2.json()['MRData']['StandingsTable']['StandingsLists']

            # Calendar
            schedule = fastf1.get_event_schedule(int(year))

            self.after(0, lambda: self._draw_data(driver_data, constructor_data, schedule))
        except Exception as e:
            self.after(0, lambda: self.status.configure(text=f"❌ Error: {str(e)[:60]}"))
            self.after(0, lambda: self.load_btn.configure(state="normal"))

    def _draw(self, no_color_updates=False, driver_data, constructor_data, schedule):
        ...
        if driver_data:
            ...
            for i, s in enumerate(standings):
                ...
                code = driver.get('code', name[:3].upper())
                pts = s['points']
                ...
                ctk.CTkLabel(row, text=code, font=ctk.CTkFont(size=12, weight="bold"), width=40).pack(side="left")
                ctk.CTkLabel(row, text=name, font=ctk.CTkFont(size=11), text_color="#aaaaaa").pack(side="left", padx=5)
                ctk.CTkLabel(row, text=f"{pts} pts", font=ctk.CTkFont(size=12, weight="bold"),
                             text_color="white").pack(side="right", padx=15)
        else:
            ctk.CTkLabel(self.driver_frame, text="No data available for this year",
        ...
        if constructor_data:
            ...
            ctk.CTkLabel(self.constructor_frame, text="No data available for this year",
                         text_color="#555555").pack(pady=20)
        ...
        self.status.configure(text="✅ Live data loaded!")
